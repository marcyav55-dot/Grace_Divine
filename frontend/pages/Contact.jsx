...
import { WHATSAPP_NUMBER, LOCATION, SECOND_PHONE, EMAIL } from "../data/siteData";
  ...
  return (
    <div style={{ marginTop: 110 }}>
      <style>{`
        ...
        .contact-fade { animation: contactFadeUp 0.7s ease-out both; }
        .contact-input:focus {
      ...
      <section style={{ padding: "60px 24px", background: "var(--bg-light)" }}>
        <div style={{
          ...
          <div className="contact-fade" style={{
            background: "var(--bg-white)", borderRadius: 18, padding: 30,
            boxShadow: "0 8px 30px rgba(10,30,80,0.08)",
          }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)", margin: "0 0 22px" }}>
              Envoyer un message
            </h2>
            ...
            {sent ? (
              <div style={{ textAlign: "center", padding: "30px 0" }}>
                ...
                <p style={{ color: "var(--text-secondary)", fontSize: 13, marginTop: 6 }}>
                  Nous vous répondrons rapidement, ou contactez-nous directement sur WhatsApp.
                </p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <input className="contact-input" style={inputStyle} type="text" name="nom" placeholder="Votre nom"
                  value={form.nom} onChange={handleChange} required />
                <input className="contact-input" style={inputStyle} type="tel" name="telephone" placeholder="Votre téléphone"
                  value={form.telephone} onChange={handleChange} required />
                <textarea className="contact-input" style={{ ...inputStyle, minHeight: 130, resize: "vertical" }}
          ...
          {/* ─── Colonne droite : WhatsApp, appel, infos ──────────────────── */}
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>

            <a
              href={`https://wa.me/${WHATSAPP_NUMBER}`}
              target="_blank" rel="noopener noreferrer"
              className="contact-fade contact-card"
              style={{
            ...
            <a
              href={`tel:${SECOND_PHONE.replace(/\s/g, "")}`}
              className="contact-fade contact-card"
              style={{
                display: "flex", alignItems: "center", gap: 14,
                ...
                color: "#fff", textDecoration: "none",
                borderRadius: 16, padding: "22px 24px",
                boxShadow: "0 8px 26px rgba(29,78,216,0.35)",
              }}
            >
              <svg width="30" height="30" viewBox="0 0 24 24" fill="currentColor">
              ...
              <div>
                ...
                <div style={{ fontSize: 13, opacity: 0.9 }}>{SECOND_PHONE}</div>
              </div>
            </a>

            <div className="contact-fade contact-card" style={{
              background: "var(--bg-white)", borderRadius: 16, padding: 24,
